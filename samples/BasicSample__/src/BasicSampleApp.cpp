...
void BasicSampleApp::draw()
{
    ...
    std::map<xtract_features_,ciXtract::FeatureCallback>::iterator it;
    ciXtract::FeatureCallback f;
    ...
    for( it = mLibXtract->mCallbacks.begin(); it != mLibXtract->mCallbacks.end(); ++it )
    {
        ...
        if ( f.type == ciXtract::SCALAR_FEATURE )
        {
...
void BasicSampleApp::initGui()
{
    ...
    for( it = mLibXtract->mCallbacks.begin(); it != mLibXtract->mCallbacks.end(); ++it )
    {
        ...
        mParams->addButton( name, std::bind( &ciXtract::toggleFeature, mLibXtract, it->first ) );
    }
}
